#!/usr/bin/env bash
#
# run-booter-detached.sh — пробный dry-load Booter на SEC2 БЕЗ ребута, с возвратом экрана.
# Аналог run-fwsec-detached.sh, но гоняет tools/booter_run_linux (фаза 3 задачи 6).
#
# ЗАПУСКАТЬ ОТЦЕПЛЕННО:
#   sudo systemd-run --unit=rtx-booter --collect bash /ABS/PATH/tools/run-booter-detached.sh
# Результат: tools/booter-run.log ; маркер: tools/booter-DONE
set -u
export PATH=/usr/bin:/bin:/usr/sbin:/sbin:$PATH   # для zstd (распаковка блоба в харнессе)

DIR="$(cd "$(dirname "$0")" && pwd)"
LOG="$DIR/booter-run.log"
HARNESS_SRC="$DIR/booter_run_linux"
HARNESS="/tmp/booter_run_linux"
BDF="0000:01:00.0"
AUD="0000:01:00.1"
...
restore_gui() {
    ...
    touch "$DIR/booter-DONE"
}
trap restore_gui EXIT

echo "=== run-booter-detached $(date -u +%FT%TZ) ==="
echo "kernel: $(uname -r)"
