...
- 🟢 **HW** — подтверждено логом с реального железа. **Сейчас таких нет.**
...
Реально подтверждены лишь два факта: (1) код компилируется (CI-лог);
(2) PCI-идентификаторы карты сняты из **Windows** (`Get-PnpDevice`), не нашим кодом
и не на macOS. `docs/hw-dumps/` — пусто.
...
**Метрика слоя 1 (чтение chip ID) — код написан и совпадает с исходником (📄 SRC),
но НЕ исполнялся на железе (🟢 HW отсутствует). На реальной карте `PMC_BOOT_0` не
читался.**
