...
| 3 память (GMMU) | VRAM через RPC | 🟢 **ПРОХОДЫ A+B+C НА ЖЕЛЕЗЕ** (2026-06-30..07-01): A — двусторонний RPC + `GET_GSP_STATIC_INFO` (карта 5 FB-регионов) + `GSP_RM_ALLOC` client→device→subdevice; B — `RM_CONTROL`/`FB_GET_INFO_V2` (RAM=12282 МиБ) + `FERMI_VASPACE_A` (корень GMMU); C — `ALLOC_MEMORY`/`NV01_MEMORY_LIST_FBMEM` регистрация VRAM (1 МиБ) — всё `NV_OK`. `driver/gsp/gsp_rm.*`, тех-запись `docs/gsp-layer3-rpc.md`, логи `docs/hw-dumps/*layer3*`. Дальше: маппинг VRAM в VA-пространство |
