...
| 3 память (GMMU) | VRAM через RPC | 🟢 **ПРОХОДЫ A+B НА ЖЕЛЕЗЕ**: A (2026-06-30) — двусторонний RPC + `GET_GSP_STATIC_INFO` (карта 5 FB-регионов, вершина=12282 МиБ) + `GSP_RM_ALLOC` client→device→subdevice; B (2026-07-01) — `RM_CONTROL`/`FB_GET_INFO_V2` (RAM=12282 МиБ, BAR1=256 МиБ) + `FERMI_VASPACE_A` (корень GMMU) — всё `NV_OK`. `driver/gsp/gsp_rm.*`, тех-запись `docs/gsp-layer3-rpc.md`, логи `docs/hw-dumps/2026063{0,}*layer3*` + `20260701-rtx4070s-layer3-passB-OK.log`. Дальше: аллокация VRAM-объекта + маппинг |
